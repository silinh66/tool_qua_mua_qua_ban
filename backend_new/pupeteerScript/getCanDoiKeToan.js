...
const CONFIG = {
  ...
  balanceSheetEndpoint:
    "fundamental.fiintrade.vn/FinancialStatement/GetBalanceSheet",
};
...
async function scrapeFinancials(page, symbol, year, quarter) {
  console.log(`\n=== Scraping ${symbol} for Q${quarter}/${year} ===`);
  await page.goto(CONFIG.baseUrl, { waitUntil: "networkidle0" });
  ...
  try {
    ...
    if (error.name === "TimeoutError") {
      console.log(
        `Timed out waiting for fiintrade data request: symbol=${symbol}`
      );
      return;
    } else {
  ...
  const rows = matched.map((item) => [symbol, ...Object.values(item)]);
  console.log(`  ✅ Parsed ${rows.length} rows, inserting to DB...`);
  await queryMySQL("INSERT INTO can_doi_ke_toan VALUES ?", [rows]);
...
(async () => {
  ...
  for (const { Symbol } of toProcess) {
    ...
    await scrapeFinancials(page, Symbol, 2025, 1);
    await delay(1000);
  }
